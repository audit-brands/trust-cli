...
export class TrustBenchmarkSystem {
  ...
  async runBenchmark(
    ...
    if (
      !this.client.isModelLoaded() ||
      this.client.getModelInfo()?.name !== model.name
    ) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      for (const test of testsToRun) {
        ...
        try {
          ...
          console.log(
            `  ${status} ${result.tokensPerSecond.toFixed(1)} t/s (${result.responseTime}ms)`,
          );
        } catch (error) {
  ...
  generateInsights(reports: Map<string, BenchmarkReport>): {
    ...
    const fastest = reportArray.reduce((prev, current) =>
      current.summary.averageTokensPerSecond >
      prev.summary.averageTokensPerSecond
        ? current
        : prev,
    );
    ...
    const mostEfficient = reportArray.reduce((prev, current) => {
      const prevEfficiency =
        prev.summary.averageTokensPerSecond / this.getAverageMemoryUsage(prev);
      const currentEfficiency =
        current.summary.averageTokensPerSecond /
        this.getAverageMemoryUsage(current);
  ...
  private generateReport(
    ...
    const recommendedUseCase = this.determineRecommendedUseCase(
  ...
  private createStandardTests(): BenchmarkTest[] {
    return [
      ...
      },
      {
        ...
        prompt:
          'Summarize the key benefits of using local AI models instead of cloud-based services for privacy-sensitive applications. Consider factors like data security, latency, costs, and control over the inference process. Provide a comprehensive analysis.',
  ...
  private findBestForTask(
    ...
    for (const report of reports) {
      const taskResult = report.results.find(
        (r) => r.testId === taskId && r.success,
      );
  ...
  private generateRecommendations(reports: BenchmarkReport[]): string[] {
    ...
    for (const report of reports) {
      ...
      if (speed > 50 && reliability > 0.9) {
        recommendations.push(
          `${report.modelName}: Excellent for production use - high speed and reliability`,
        );
      } else if (speed > 20 && reliability > 0.8) {
        recommendations.push(
          `${report.modelName}: Good for general use - balanced performance`,
        );
      } else if (speed < 10) {
        recommendations.push(
          `${report.modelName}: Consider upgrading hardware or using a smaller model`,
        );
      } else if (reliability < 0.7) {
        recommendations.push(
          `${report.modelName}: May need troubleshooting - low success rate`,
        );
      }
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'No specific recommendations - all models performed adequately',
      );
    }
