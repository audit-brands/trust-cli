...
export class LoRAFineTuningDemo {
  ...
  private async collectErrors(): Promise<void> {
    ...
    try {
      ...
      console.log(
        '   🎯 Error types: parse_error, wrong_tool, wrong_args, validation_error',
      );
    } catch (error) {
  ...
  private async analyzeModels(): Promise<void> {
    try {
      ...
      for (const { model, weaknesses, priority } of weakModels) {
        ...
        console.log(
          `        Parameters: ${model.parameters}, RAM: ${model.ramRequirement}`,
        );
      }
      ...
      console.log(
        `      - Address JSON generation issues for parse_error problems`,
      );
  ...
  private async generateDatasets(): Promise<void> {
    try {
      console.log(
        '   📚 Generating training datasets from error collection...',
      );
      ...
      console.log(
        '      - Source: Error collection from function call failures',
      );
      console.log(
        '      - Focus: JSON generation, tool selection, argument formatting',
      );
      console.log(
        '      - Quality: High (>200 samples from diverse error types)',
      );
      console.log('      - Domain: Function calling and structured output');

      console.log('   ✅ Training datasets generated successfully');
      console.log(
        '   💾 Saved in both JSON and JSONL formats for compatibility',
      );
    } catch (error) {
  ...
  private async createConfigurations(): Promise<void> {
    try {
      ...
      for (const model of models.slice(0, 3)) {
        ...
        console.log(
          `      - Batch Size: ${config.batchSize}, Epochs: ${config.epochs}`,
        );
        console.log(
          `      - Target Modules: ${config.targetModules.join(', ')}`,
        );

        // Show reasoning for configuration choices
        if (model.parameters?.includes('1.5B')) {
          console.log(
            `      💡 Optimized for small model: Lower rank/alpha for efficiency`,
          );
        } else if (model.parameters?.includes('7B')) {
          console.log(
            `      💡 Optimized for large model: Higher rank/alpha, smaller batch`,
          );
        }
      }
  ...
  private async simulateTraining(): Promise<void> {
    try {
      ...
      console.log(
        `   📊 Configuration: Rank=${config.rank}, Alpha=${config.alpha}, Epochs=${config.epochs}`,
      );
      ...
      if (job.resultAdapter) {
        ...
        console.log(
          `        • Loss: ${job.resultAdapter.performance.loss.toFixed(3)}`,
        );
        console.log(
          `        • Perplexity: ${job.resultAdapter.performance.perplexity.toFixed(2)}`,
        );
        console.log(
  ...
  private async evaluateResults(): Promise<void> {
    try {
      ...
      for (const adapter of adapters) {
        ...
        console.log(
          `        • Accuracy: ${adapter.performance.accuracy.toFixed(3)} (${adapter.performance.accuracy > 0.85 ? '✅ Good' : '⚠️ Needs improvement'})`,
        );
        console.log(
          `        • Loss: ${adapter.performance.loss.toFixed(3)} (${adapter.performance.loss < 0.5 ? '✅ Low' : '⚠️ High'})`,
        );
        console.log(
          `        • Benchmark Score: ${adapter.performance.benchmarkScore.toFixed(3)} (${adapter.performance.benchmarkScore > 0.8 ? '✅ Good' : '⚠️ Needs improvement'})`,
        );
        console.log(
  ...
  private async generateReports(): Promise<void> {
    try {
      ...
      const summaryLines = reportLines.filter(
        (line) =>
      ...
      console.log(
        '      - LoRA fine-tuning can significantly improve weak model performance',
      );
      console.log('      - Error collection provides valuable training data');
      console.log(
        '      - Model-specific configurations optimize training efficiency',
      );
      console.log(
      ...
      console.log(
        '      - Create datasets: trust lora dataset create --model <name>',
      );
      console.log(
        '      - Start training: trust lora train --model <name> --dataset <id>',
      );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  demo
    ...
    .then(() => {
      ...
      console.log(
        '📚 Documentation and examples are available in the codebase.',
      );
