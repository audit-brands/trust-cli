...
export class PerformanceBenchmark {
    ...
    const speedSuite: BenchmarkSuite = {
      ...
      tests: [
        ...
        },
        {
          ...
          prompt:
            'Summarize in one sentence: Machine learning is a subset of artificial intelligence that enables computers to learn and improve from experience without being explicitly programmed.',
          expectedTokens: 30,
          timeout: 10000,
          category: 'speed',
          difficulty: 'medium',
    ...
    const qualitySuite: BenchmarkSuite = {
      ...
      tests: [
        ...
        },
        {
          ...
          prompt:
            'Write a short story (2-3 paragraphs) about a robot learning to paint.',
          expectedTokens: 200,
          timeout: 45000,
          category: 'quality',
          difficulty: 'hard',
    ...
    const codingSuite: BenchmarkSuite = {
      ...
      tests: [
        ...
        },
        {
          ...
          prompt:
            'Design an algorithm to find the longest palindromic substring in a string. Provide both the algorithm description and Python implementation.',
          expectedTokens: 300,
          timeout: 60000,
          category: 'coding',
          difficulty: 'hard',
    ...
    const reasoningSuite: BenchmarkSuite = {
      ...
      tests: [
        ...
        },
        {
          ...
          prompt:
            'A train travels 240 miles in 4 hours. If it maintains the same speed, how long will it take to travel 420 miles? Show your work.',
          expectedTokens: 100,
          timeout: 25000,
          category: 'reasoning',
          difficulty: 'medium',
        ...
        },
        {
          ...
          prompt:
            'Three friends - Alice, Bob, and Carol - have different amounts of money. Alice has twice as much as Bob. Carol has $10 more than Alice. Together they have $70. How much does each person have?',
          expectedTokens: 150,
          timeout: 40000,
          category: 'reasoning',
          difficulty: 'hard',
  ...
  private generateModelSummary(
    ...
    const averageSpeed =
      successfulResults.length > 0
        ? successfulResults.reduce(
            (sum, r) => sum + r.metrics.tokensPerSecond,
            0,
    ...
    const overallScore =
      speedScore * 0.4 + qualityScore * 0.4 + reliabilityScore * 0.2;

    // Calculate efficiency (tokens per second per GB RAM requirement)
    const model = this.modelManager
      .listAvailableModels()
      .find((m) => m.name === modelName);
    const ramRequirement = model
      ? parseFloat(model.ramRequirement.replace('GB', ''))
      : 4;
  ...
  private generateRecommendations(
    ...
    const bestForCoding = summaries.reduce((prev, current) =>
      current.bestCategories.includes('coding') &&
      current.overallScore > prev.overallScore
        ? current
        : prev,
    );

    const bestForReasoning = summaries.reduce((prev, current) =>
      current.bestCategories.includes('reasoning') &&
      current.overallScore > prev.overallScore
        ? current
        : prev,
    );
  ...
  generateTextReport(report: BenchmarkReport): string {
    ...
    // Model Rankings
    const sortedModels = [...report.models].sort(
