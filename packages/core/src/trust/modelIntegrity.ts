...
export class ModelIntegrityChecker {
    ...
    try {
      ...
      const hashes = await this.computeFileHashes(
        ...
        (processed, total) => {
          ...
          onProgress?.(
            `Computing hash... ${Math.round((processed / total) * 100)}%`,
            hashProgress,
          );
        },
      );
      ...
      const isPending =
        expectedHash === 'pending_verification' ||
      ...
      if (
        trustedInfo &&
        (trustedInfo.sha256 === 'pending_verification' ||
          trustedInfo.sha256 === 'sha256:pending')
      ) {
  ...
  async generateIntegrityReport(
    ...
    return {
      ...
      signatureValid: trustedInfo
        ? hashes.sha256 === trustedInfo.sha256
        : undefined,
    };
  }
  ...
  async verifyAllModels(
    ...
    try {
      ...
      for (const file of ggufFiles) {
        ...
        onProgress?.(
          modelName,
          result.valid ? 'Verified ✓' : `Failed: ${result.reason}`,
        );
      }
    } catch (error) {
