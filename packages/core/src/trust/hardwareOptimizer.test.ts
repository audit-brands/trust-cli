...
describe('HardwareOptimizer', () => {
  ...
  describe('model suitability analysis', () => {
    it('should rank models by suitability score', () => {
      const recommendations =
        hardwareOptimizer.analyzeModelSuitability(sampleModels);

      expect(recommendations).toHaveLength(3);
      expect(recommendations[0].suitabilityScore).toBeGreaterThanOrEqual(
        recommendations[1].suitabilityScore,
      );
      expect(recommendations[1].suitabilityScore).toBeGreaterThanOrEqual(
        recommendations[2].suitabilityScore,
      );
    });
    ...
    it('should give high scores to models that fit well in available memory', () => {
      ...
      const lightweightRec = recommendations.find(
        (r) => r.model.name === 'lightweight-model',
      );
      const mediumRec = recommendations.find(
        (r) => r.model.name === 'medium-model',
      );
      const largeRec = recommendations.find(
        (r) => r.model.name === 'large-model',
      );
    ...
    it('should generate warnings for models requiring too much memory', () => {
      ...
      const recommendations =
        lowMemOptimizer.analyzeModelSuitability(sampleModels);

      const largeModelRec = recommendations.find(
        (r) => r.model.name === 'large-model',
      );
      expect(largeModelRec?.warnings).toContain(
    ...
    it('should provide performance estimates', () => {
      ...
      recommendations.forEach((rec) => {
        ...
        expect(rec.performanceEstimate.cpuUtilization).toBeGreaterThanOrEqual(
          0,
        );
  ...
  describe('optimization recommendations', () => {
    it('should generate appropriate recommendations for low memory systems', () => {
      ...
      const lowMemOptimizer = new HardwareOptimizer(mockPerformanceMonitor);
      const recommendations =
        lowMemOptimizer.generateOptimizationRecommendations();

      const memoryRec = recommendations.find((r) =>
        r.title.includes('Low Available Memory'),
      );
    ...
    it('should recommend better quantization for high memory systems', () => {
      ...
      const recommendations =
        highMemOptimizer.generateOptimizationRecommendations();

      const qualityRec = recommendations.find((r) =>
        r.title.includes('Higher Quality Models'),
      );
    ...
    it('should provide platform-specific recommendations', () => {
      ...
      const macRecommendations =
        macOptimizer.generateOptimizationRecommendations();

      const metalRec = macRecommendations.find((r) =>
        r.title.includes('Metal'),
      );
