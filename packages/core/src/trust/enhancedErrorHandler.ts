...
export interface EnhancedError {
  type: 'model_not_found' | 'backend_unavailable' | 'insufficient_resources' | 'configuration_invalid' | 'network_error' | 'permission_denied' | 'routing_failed' | 'generic';
...
interface ErrorPattern {
  ...
  solutionGenerator: (match: RegExpMatchArray, context?: any) => ErrorSolution[];
  relatedCommands: string[];
  documentation?: string;
  estimatedFixTime: string;
}
...
export class EnhancedErrorHandler {
   ...
   */
  async processError(error: Error | string, context?: Record<string, any>): Promise<EnhancedError> {
   ...
   */
  async processErrors(errors: (Error | string)[], context?: Record<string, any>): Promise<EnhancedError[]> {
    ...
    return enhancedErrors.sort((a, b) => severityOrder[b.severity] - severityOrder[a.severity]);
  }
  ...
  getContextualHelp(errorType: EnhancedError['type']): string {
    const helpMap: Record<EnhancedError['type'], string> = {
      model_not_found: 'This error occurs when Trust CLI cannot locate the specified model. This usually means the model needs to be downloaded or the name is incorrect.',
      backend_unavailable: 'This error indicates that a required backend (Ollama, HuggingFace, etc.) is not running or properly configured.',
      insufficient_resources: 'This error means your system lacks the necessary resources (RAM, disk space, etc.) to complete the operation.',
      configuration_invalid: 'This error indicates there is an issue with your Trust CLI configuration that needs to be resolved.',
      network_error: 'This error occurs when Trust CLI cannot connect to required network services or download resources.',
      permission_denied: 'This error indicates insufficient permissions to access files or execute commands.',
      routing_failed: 'This error occurs when the intelligent model routing system cannot find a suitable model for your request.',
      generic: 'This is a general error that could not be specifically classified. Check the error details for more information.',
    };
  ...
  private initializeErrorPatterns(): ErrorPattern[] {
    return [
      ...
      {
        pattern: /model ['"`]?([^'"`\s]+)['"`]? not found|Model ['"`]?([^'"`\s]+)['"`]? does not exist|No such model|model.*not found/i,
        type: 'model_not_found',
        severity: 'medium',
        titleTemplate: 'Model Not Found',
        descriptionTemplate: 'The requested model "{modelName}" could not be found in any enabled backend.',
        causeTemplate: 'The model may not be downloaded, or the model name might be incorrect.',
        solutionGenerator: (match) => this.generateModelNotFoundSolutions(match[1] || match[2] || 'unknown'),
        relatedCommands: [
          'trust model list',
      ...
      {
        pattern: /connection refused|backend not available|ollama.*not running|server.*not responding|ECONNREFUSED/i,
        type: 'backend_unavailable',
        severity: 'high',
        titleTemplate: 'Backend Service Unavailable',
        descriptionTemplate: 'A required backend service is not running or cannot be reached.',
        causeTemplate: 'The backend service (Ollama, HuggingFace API, etc.) is not started or properly configured.',
      ...
      {
        pattern: /out of memory|insufficient.*memory|not enough.*space|disk.*full|RAM.*exceeded/i,
        type: 'insufficient_resources',
        severity: 'high',
        titleTemplate: 'Insufficient System Resources',
        descriptionTemplate: 'The system lacks sufficient resources to complete this operation.',
        causeTemplate: 'Available RAM, disk space, or other system resources are below required thresholds.',
      ...
      {
        pattern: /config.*invalid|configuration.*error|missing.*api.*key|invalid.*token|authentication.*failed/i,
        type: 'configuration_invalid',
        severity: 'medium',
        titleTemplate: 'Configuration Issue',
        descriptionTemplate: 'There is an issue with your Trust CLI configuration.',
        causeTemplate: 'Configuration file is missing, corrupted, or contains invalid values.',
      ...
      // Network errors
      {
        pattern: /network.*error|timeout|dns.*resolution|certificate.*error|ssl.*error|fetch.*failed/i,
        type: 'network_error',
        severity: 'medium',
        titleTemplate: 'Network Connection Issue',
        descriptionTemplate: 'Trust CLI cannot establish a network connection.',
        causeTemplate: 'Network connectivity issues, DNS problems, or firewall restrictions.',
      ...
      // Permission errors
      {
        ...
        descriptionTemplate: 'Trust CLI lacks sufficient permissions to complete this operation.',
        causeTemplate: 'File or directory permissions, or insufficient user privileges.',
      ...
      // Routing failures
      {
        pattern: /routing.*failed|no suitable model|model selection.*failed|intelligent routing.*error/i,
        type: 'routing_failed',
        severity: 'medium',
        titleTemplate: 'Model Routing Failed',
        descriptionTemplate: 'The intelligent routing system could not find a suitable model for your request.',
        causeTemplate: 'No available models meet the criteria, or system constraints are too restrictive.',
  ...
  private buildEnhancedError(pattern: ErrorPattern, match: RegExpMatchArray, context: any): EnhancedError {
    ...
    return {
      ...
      description: pattern.descriptionTemplate.replace('{modelName}', modelName),
  ...
  private createGenericError(errorMessage: string, context?: Record<string, any>): EnhancedError {
    return {
      type: 'generic',
      severity: 'medium',
      title: 'Unexpected Error',
      description: 'An unexpected error occurred that could not be automatically classified.',
      cause: errorMessage,
      context: {
        ...context,
        originalError: errorMessage,
      ...
      solutions: [
        {
          ...
          description: 'Verify that all services are running and the system is healthy.',
    ...
    return [
      {
        ...
        description: 'Look for models with similar names or check available models.',
        commands: [
          'trust model list',
          'trust model-enhanced discover',
  ...
  private generatePermissionSolutions(): ErrorSolution[] {
    return [
      ...
      },
      {
        ...
        description: 'Add your user to groups that have the necessary permissions.',
        commands: [
