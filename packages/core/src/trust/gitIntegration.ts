...
export class TrustGitIntegration {
  ...
  async getRepoInfo(): Promise<GitRepoInfo> {
    try {
      ...
      // Get current branch
      const branch = this.executeGitCommand(
      ...
      try {
        ...
        if (remoteName) {
          remoteUrl = this.executeGitCommand(
  ...
  async analyzeChanges(
    ...
    if (chatSession) {
      try {
        const analysis = await this.analyzeChangesWithAI(
          combinedDiff,
          chatSession,
        );
    ...
    return {
      filesChanged:
        repoInfo.status.staged.length + repoInfo.status.modified.length,
   ...
   */
  async prepareRepositoryContext(
    options: {
    ...
    if (
      focusOnChanges &&
      (repoInfo.status.staged.length > 0 || repoInfo.status.modified.length > 0)
    ) {
    ...
    try {
      ...
      for (const file of importantFiles) {
        try {
          await this.contextManager.addFile(
            file,
  ...
  private assessComplexity(stats: {
    linesAdded: number;
    linesRemoved: number;
  }): 'low' | 'medium' | 'high' {
  ...
  private generateBasicSuggestions(stats: {
    linesAdded: number;
    linesRemoved: number;
  }): string[] {
    ...
    if (stats.linesAdded > 100) {
      suggestions.push(
        'Large number of additions - consider breaking into smaller commits',
      );
    }

    if (stats.linesRemoved > 50) {
      suggestions.push(
        'Significant code removal - ensure functionality is preserved',
      );
    }

    if (stats.linesAdded > stats.linesRemoved * 3) {
      suggestions.push(
        'Consider if some additions could be refactored or simplified',
      );
    }
