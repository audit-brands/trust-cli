...
function formatInColumns(
  ...
  const _maxKeyLength = Math.max(...items.map((item) => item.key.length));
  const _minDescLength = Math.min(
...
export const Help: React.FC<Help> = ({ commands }) => {
  ...
  return (
    ...
    >
      ...
      <Text bold color={Colors.AccentPurple}>
        Trust CLI Help{' '}
        {terminalWidth >= 60 ? `(Terminal: ${terminalWidth} cols)` : ''}
      </Text>
      ...
      {terminalWidth >= 80 && (
        <>
          ...
          <Text dimColor color={Colors.Gray}>
            Tip: Use full terminal width for better experience. Currently using{' '}
