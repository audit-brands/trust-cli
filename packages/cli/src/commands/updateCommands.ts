...
export interface UpdateCommandArgs {
  action: 'analyze' | 'plan' | 'execute' | 'schedule' | 'history' | 'policy' | 'status' | 'rollback';
  subaction?: string;
  batchId?: string;
...
export class UpdateCommandHandler {
    ...
    try {
      ...
      Object.entries(grouped).forEach(([ecosystem, ecosystemUpdates]) => {
        ...
        ecosystemUpdates.forEach(update => {
          ...
          console.log(`      Current: ${update.currentVersion} → Target: ${update.targetVersion}`);
          console.log(`      Type: ${update.updateType} | Risk: ${riskIcon} ${update.riskLevel} | Reason: ${reasonIcon} ${update.reason}`);
          ...
          if (args.verbose) {
            ...
            if (update.dependents.length > 0) {
              console.log(`      🔗 Dependents: ${update.dependents.join(', ')}`);
            }
          }
        });
      });
      ...
      console.log('\n💡 Next Steps:');
      console.log('   trust update plan                    # Create update plan');
  ...
  private async handlePlanCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    try {
      ...
      if (planned.length === 0) {
        ...
        if (updates.length > 0) {
          ...
          filtered.slice(0, 5).forEach(update => {
            console.log(`   ${update.name}: ${update.currentVersion} → ${update.targetVersion} (${update.updateType})`);
          });
      ...
      if (automated.length > 0) {
        ...
        automated.forEach(update => {
          const riskIcon = this.getRiskIcon(update.riskLevel);
          const reasonIcon = this.getReasonIcon(update.reason);
          console.log(`   ${reasonIcon} ${update.name}: ${update.currentVersion} → ${update.targetVersion} (${riskIcon} ${update.riskLevel})`);
        });
      }
      ...
      // Risk assessment
      const riskCounts = planned.reduce((acc, u) => {
        acc[u.riskLevel]++;
        return acc;
      ...
      console.log('   trust update execute --security-only # Security updates only');
  ...
  private async handleExecuteCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    try {
      ...
      if (args.securityOnly) {
        ...
        console.log(`🛡️  Security-only mode: ${planned.length} security updates`);
      }

      if (args.ecosystem) {
        planned = planned.filter(u => u.ecosystem === args.ecosystem);
        console.log(`📦 Ecosystem filter (${args.ecosystem}): ${planned.length} updates`);
      }

      if (args.package) {
        planned = planned.filter(u => u.name === args.package);
        console.log(`📦 Package filter (${args.package}): ${planned.length} updates`);
      }
      ...
      if (args.dryRun) {
        ...
        planned.forEach(update => {
          console.log(`   Would update ${update.name}: ${update.currentVersion} → ${update.targetVersion}`);
        });
        return;
      }
  ...
  private async handleScheduleCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    if (args.subaction === 'enable') {
      ...
      if (policy.scheduling.enabled) {
        console.log(`   Schedule: ${policy.scheduling.cron}`);
        console.log(`   Maintenance Mode: ${policy.scheduling.maintenance ? '✅' : '❌'}`);
      }
      
    } else {
      console.log('⏰ Schedule Management Commands:');
      console.log('   trust update schedule enable --schedule "0 2 * * 1"  # Enable weekly updates');
      console.log('   trust update schedule disable                         # Disable scheduling');
      console.log('   trust update schedule status                          # Show current status');
  ...
  private async handleHistoryCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    try {
      ...
      history.slice(0, 10).forEach(batch => {
        ...
        console.log(`   Updates: ${batch.summary.total} total, ${batch.summary.successful} successful, ${batch.summary.failed} failed`);
        ...
        if (args.verbose) {
          ...
          batch.results.slice(0, 3).forEach(result => {
            ...
            console.log(`     ${resultIcon} ${result.package}: ${result.fromVersion} → ${result.toVersion}`);
          });
      ...
      console.log('\n💡 History Commands:');
      console.log('   trust update history --verbose      # Show detailed history');
      console.log('   trust update rollback <batch-id>    # Rollback a batch');

    } catch (error) {
      console.error(`❌ Failed to load history: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async handlePolicyCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    if (args.subaction === 'show') {
      ...
      console.log(`   Security Only: ${policy.autoUpdate.securityOnly ? '✅' : '❌'}`);
      console.log(`   Patch Level: ${policy.autoUpdate.patchLevel ? '✅' : '❌'}`);
      console.log(`   Minor Level: ${policy.autoUpdate.minorLevel ? '✅' : '❌'}`);
      console.log(`   Major Level: ${policy.autoUpdate.majorLevel ? '✅' : '❌'}`);
      ...
      if (policy.exclusions.packages.length > 0) {
        ...
        policy.exclusions.packages.forEach(pkg => {
          const reason = policy.exclusions.reasons[pkg] || 'No reason specified';
      ...
      console.log('   trust update policy show                    # Show current policy');
      console.log('   trust update policy set --auto-update      # Enable auto updates');
      console.log('   trust update policy set --security-only    # Security updates only');
      console.log('   trust update policy set --test-command     # Set test command');
    }
  }

  private async handleStatusCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    try {
      ...
      console.log(`   Auto Updates: ${policy.autoUpdate.enabled ? '✅ Enabled' : '❌ Disabled'}`);
      console.log(`   Scheduled Updates: ${policy.scheduling.enabled ? '✅ Enabled' : '❌ Disabled'}`);
      console.log(`   Testing: ${policy.testing.required ? '✅ Required' : '❌ Optional'}`);
      console.log(`   Rollback: ${policy.rollback.enabled ? '✅ Enabled' : '❌ Disabled'}`);

      console.log('\n📊 Current Status:');
      console.log(`   Available Updates: ${updates.length}`);
      console.log(`   Planned Updates: ${planned.length}`);
      console.log(`   Security Updates: ${updates.filter(u => u.reason === 'security').length}`);
      console.log(`   Breaking Changes: ${updates.filter(u => u.breaking).length}`);

      if (history.length > 0) {
        const lastBatch = history[0];
        console.log('\n📈 Last Update:');
        console.log(`   Date: ${new Date(lastBatch.timestamp).toLocaleDateString()}`);
        console.log(`   Status: ${lastBatch.status}`);
        console.log(`   Updates: ${lastBatch.summary.successful}/${lastBatch.summary.total} successful`);
      }
  ...
  private async handleRollbackCommand(args: UpdateCommandArgs): Promise<void> {
    ...
    try {
      ...
      console.log('💡 This would restore dependency files from the batch backup');
  ...
  private async interactiveSelection(updates: DependencyUpdate[]): Promise<DependencyUpdate[]> {
    ...
    updates.forEach((update, index) => {
      const riskIcon = this.getRiskIcon(update.riskLevel);
      console.log(`   ${index + 1}. ${update.name}: ${update.currentVersion} → ${update.targetVersion} (${riskIcon} ${update.riskLevel})`);
    });
  ...
  private setupEventHandlers(): void {
    ...
    this.updater.on('update-progress', (progress: { batch: UpdateBatch; current: number; total: number }) => {
    ...
    this.updater.on('batch-completed', (batch: UpdateBatch) => {
      console.log(`✅ Batch ${batch.id} completed: ${batch.summary.successful}/${batch.summary.total} successful`);
    });
  }
...
export async function handleUpdateCommand(args: UpdateCommandArgs): Promise<void> {
