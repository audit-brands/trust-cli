...
export interface ExtensionCommandArgs {
  action: 'search' | 'install' | 'uninstall' | 'update' | 'list' | 'enable' | 'disable' | 'info' | 'marketplace';
  subaction?: string;
  name?: string;
  query?: string;
  category?: string;
...
export class ExtensionCommandHandler {
  ...
  private async handleSearchCommand(args: ExtensionCommandArgs): Promise<void> {
    ...
    try {
      ...
      extensions.forEach(ext => {
        ...
        console.log(`   Rating: ${'★'.repeat(Math.floor(ext.rating))}${'☆'.repeat(5 - Math.floor(ext.rating))} (${ext.rating}/5)`);
        ...
        if (args.verbose) {
          ...
          console.log(`   Published: ${new Date(ext.publishDate).toLocaleDateString()}`);
          console.log(`   Updated: ${new Date(ext.updateDate).toLocaleDateString()}`);
      ...
      console.log('💡 Use `trust extensions info <name>` for detailed information');
  ...
  private async handleInstallCommand(args: ExtensionCommandArgs): Promise<void> {
    if (!args.name) {
    ...
    try {
      ...
      console.log(`📁 Installed to: ${path.join(this.extensionsDir, args.name)}`);
      console.log('\n💡 Use `trust extensions list` to see all installed extensions');
      console.log('💡 Use `trust extensions enable <name>` if the extension is disabled');
  ...
  private async handleUninstallCommand(args: ExtensionCommandArgs): Promise<void> {
    if (!args.name) {
  ...
  private async handleUpdateCommand(args: ExtensionCommandArgs): Promise<void> {
    ...
    try {
      if (args.all) {
        ...
        console.log('💡 Use `trust extensions update <name>` to update a specific extension');
      }
  ...
  private async handleListCommand(args: ExtensionCommandArgs): Promise<void> {
    ...
    if (extensions.length === 0) {
      ...
      console.log('💡 Use `trust extensions marketplace` to see featured extensions');
      return;
    }
    ...
    console.log('💡 Use `trust extensions enable/disable <name>` to control extensions');
  }
  ...
  private async handleEnableCommand(args: ExtensionCommandArgs): Promise<void> {
    ...
    try {
      ...
      console.error(`❌ Failed to enable extension: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  private async handleDisableCommand(args: ExtensionCommandArgs): Promise<void> {
    if (!args.name) {
      throw new Error('Extension name is required');
    }
    ...
    try {
      ...
      console.error(`❌ Failed to disable extension: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async handleInfoCommand(args: ExtensionCommandArgs): Promise<void> {
    ...
    try {
      // Check if installed
      const installed = this.manager.getInstalledExtensions().find(ext => ext.manifest.name === args.name);
      ...
      if (installed) {
        ...
        const marketplace = await this.manager.searchExtensions({ query: args.name, limit: 1 });
        const found = marketplace.find(ext => ext.manifest.name === args.name);
        
        if (found) {
          console.log('\n🏪 Marketplace Extension:');
          this.displayMarketplaceExtensionInfo(found);
        } else {
          console.log(`❌ Extension '${args.name}' not found in installed extensions or marketplace`);
          return;
        }
      }

    } catch (error) {
      console.error(`❌ Failed to get extension info: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  private async handleMarketplaceCommand(args: ExtensionCommandArgs): Promise<void> {
    console.log('🏪 Trust CLI Extension Marketplace');
    console.log('═══════════════════════════════════════════════════════════');
    ...
    try {
      ...
      console.error(`❌ Failed to load marketplace: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  private displayExtensionInfo(extension: InstalledExtension, verbose = false): void {
    ...
    console.log(`      Author: ${extension.manifest.author} | License: ${extension.manifest.license}`);
    
    if (verbose) {
      console.log(`      Installed: ${new Date(extension.installDate).toLocaleDateString()}`);
      console.log(`      Path: ${extension.installPath}`);
      
      if (extension.lastUpdate) {
        console.log(`      Last Update: ${new Date(extension.lastUpdate).toLocaleDateString()}`);
      }
  ...
  private displayDetailedExtensionInfo(extension: InstalledExtension): void {
    ...
    if (extension.manifest.engines) {
      console.log(`Engines: Trust CLI ${extension.manifest.engines['trust-cli']}`);
    }
  ...
  private displayMarketplaceExtensionInfo(extension: MarketplaceExtension): void {
    const badges = [];
    ...
    console.log(`   ${extension.manifest.description}`);
    console.log(`   Author: ${extension.manifest.author}`);
    console.log(`   Rating: ${'★'.repeat(Math.floor(extension.rating))}${'☆'.repeat(5 - Math.floor(extension.rating))} (${extension.rating}/5)`);
    ...
    const isInstalled = this.manager.isExtensionInstalled(extension.manifest.name);
    if (isInstalled) {
      console.log('   ✅ Installed');
    } else {
      console.log(`   📥 Install: trust extensions install ${extension.manifest.name}`);
    }
  }
  ...
  private async displayMarketplaceOverview(): Promise<void> {
    ...
    console.log('\n🔥 Popular Extensions:');
    const popular = all.sort((a, b) => b.downloadCount - a.downloadCount).slice(0, 3);
    popular.forEach(ext => {
      console.log(`   • ${ext.manifest.name} - ${ext.downloadCount.toLocaleString()} downloads`);
    });
    
    console.log('\n💡 Marketplace Commands:');
    console.log('   trust extensions marketplace featured     # Show featured extensions');
    console.log('   trust extensions marketplace verified    # Show verified extensions');
    console.log('   trust extensions marketplace categories  # Show available categories');
    console.log('   trust extensions search <query>          # Search extensions');
  }

  private async displayCategories(): Promise<void> {
    // In a real implementation, this would come from the registry
    const categories = [
      'productivity', 'audit', 'security', 'tools', 
      'analytics', 'reporting', 'automation', 'development'
    ];
    
    console.log('\n📂 Extension Categories:');
    for (const category of categories) {
      const extensions = await this.manager.searchExtensions({ category, limit: 100 });
  ...
  private setupEventHandlers(): void {
    this.manager.on('extension-installed', (extension: InstalledExtension) => {
      console.log(`🎉 Extension ${extension.manifest.name} installed successfully`);
    });

    this.manager.on('extension-uninstalled', (extension: InstalledExtension) => {
...
export async function handleExtensionCommand(args: ExtensionCommandArgs): Promise<void> {
