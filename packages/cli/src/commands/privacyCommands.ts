...
export class PrivacyCommandHandler {
  ...
  private async showPrivacyStatus(verbose = false): Promise<void> {
    ...
    console.log('\\n⚙️  Active Settings:');
    console.log(
      `   Model Verification: ${config.privacy.modelVerification ? 'enabled' : 'disabled'}`,
    );
    console.log(
      `   Audit Logging: ${config.privacy.auditLogging ? 'enabled' : 'disabled'}`,
    );
    console.log(
      `   Prompt Logging: ${config.transparency.logPrompts ? 'enabled' : 'disabled'}`,
    );
    console.log(
      `   Response Logging: ${config.transparency.logResponses ? 'enabled' : 'disabled'}`,
    );
    console.log(
      `   Streaming: ${config.inference.stream ? 'enabled' : 'disabled'}`,
    );
  ...
  private async switchPrivacyMode(
    mode: 'strict' | 'moderate' | 'open',
  ): Promise<void> {
    const currentMode = this.privacyManager.getCurrentMode();
    ...
    try {
      ...
      if (newMode.settings.privacy?.modelVerification !== undefined) {
        console.log(
          `   Model Verification: ${newMode.settings.privacy.modelVerification ? 'enabled' : 'disabled'}`,
        );
      }
      if (newMode.settings.privacy?.auditLogging !== undefined) {
        console.log(
          `   Audit Logging: ${newMode.settings.privacy.auditLogging ? 'enabled' : 'disabled'}`,
        );
      }
      if (newMode.settings.inference?.stream !== undefined) {
        console.log(
          `   Streaming: ${newMode.settings.inference.stream ? 'enabled' : 'disabled'}`,
        );
      }
  ...
  private async listPrivacyModes(): Promise<void> {
    ...
    console.log('💡 Use "trust privacy switch <mode>" to change modes');
    console.log('💡 Use "trust privacy info <mode>" for detailed information');
  }

  private async showModeInfo(
    mode?: 'strict' | 'moderate' | 'open',
  ): Promise<void> {
    ...
    if (modeConfig.settings.transparency) {
      ...
      Object.entries(modeConfig.settings.transparency).forEach(
        ([key, value]) => {
          console.log(`     ${key}: ${value}`);
        },
      );
