...
export class StatusCommandHandler {
  ...
  private async showBackendStatus(verbose = false): Promise<void> {
    ...
    console.log(
      `   🦙 Ollama: ${ollamaEnabled ? '✅ Enabled' : '❌ Disabled'}`,
    );
    ...
    try {
      ...
      const huggingfaceEnabled =
        this.trustConfig.isBackendEnabled('huggingface');
      console.log(
        `   🤗 HuggingFace: ${huggingfaceEnabled ? '✅ Enabled' : '❌ Disabled'}`,
      );
    ...
    try {
      ...
      console.log(
        `   ☁️  Cloud: ${cloudEnabled ? '✅ Enabled' : '❌ Disabled'}`,
      );
