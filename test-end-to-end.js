...
async function testEndToEnd() {
  ...
  try {
    ...
    if (downloadedModel) {
      ...
      const integrity = await modelManager.verifyModelIntegrity(
      ...
      console.log(
        `   💾 System RAM: ${Math.floor(systemMetrics.memoryUsage.total / 1024 ** 3)}GB`,
      );
      ...
      try {
        ...
        console.log(
          `   ⚡ Simulated speed: ${(simulatedTokens / (inferenceTime / 1000)).toFixed(1)} tokens/sec`,
        );
      } catch (error) {
      ...
      try {
        ...
        const response =
          `Hello! I'm a simulated response from the Trust CLI system. ` +
      ...
      console.log(
        '   💡 Note: Current downloads are placeholder files for testing',
      );
    }
    ...
    console.log(
      `    ⏱️  Average time: ${stats.averageInferenceTime.toFixed(0)}ms`,
    );
    ...
    if (downloadedModel) {
      ...
      console.log(
        '💡 To enable real inference, download actual GGUF model files',
      );
      console.log(
        '💡 Current system uses placeholder files for development/testing',
      );
    } else {
